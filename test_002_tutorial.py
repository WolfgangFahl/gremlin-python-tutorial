...
def test_tutorial1():
  ...
  # of a vertex
  assert asString(vList)=="[v[1], v[2], v[3], v[4], v[5], v[6]]"

#gremlin> g.V(1) //(2)
#    ==>v[1]
def test_tutorial2():
   assert asString(g.V(1).toList())=="[v[1]]"

#gremlin> g.V(1).values('name') //3
#  ==>marko
def test_tutorial3():
    assert asString( g.V(1).values('name').toList())=="['marko']"

#     gremlin> g.V(1).outE('knows') //4
#    ==>e[7][1-knows->2]
#    ==>e[8][1-knows->4]
def test_tutorial4():
    assert asString(g.V(1).outE("knows").toList()) == "[e[7][1-knows->2], e[8][1-knows->4]]"

#    gremlin> g.V(1).outE('knows').inV().values('name') //5\
#    ==>vadas
#    ==>josh
def test_tutorial5():
    assert asString(g.V(1).outE("knows").inV().values("name").toList())=="['vadas', 'josh']"

#     gremlin> g.V(1).out('knows').values('name') //6\
#    ==>vadas
#    ==>josh
def test_tutorial6():
    assert asString(g.V(1).out("knows").values("name").toList())=="['vadas', 'josh']"
